...
@implementation UGCPaletteTarget

- (instancetype)initWithTargetMode:(UGCPaletteTargetMode)mode{
    self = [super init];
    if (self){
        _mode = mode;
        [self initParams];
...
- (void)configureLumaAndSaturationWithMode:(UGCPaletteTargetMode)mode{
    switch (mode) {
...
- (NSString*)getTargetKey{
    return [UGCPaletteTarget getTargetModeKey:_mode];
}


+ (NSString*)getTargetModeKey:(UGCPaletteTargetMode)mode{
    NSString *key;
    switch (mode) {
        case LIGHT_VIBRANT_PALETTE:
            key = @"light_vibrant";
