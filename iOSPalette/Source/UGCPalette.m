...
#import "UGCPalette.h"
#import "UGCPaletteSwatch.h"
#import "UGCPaletteColorUtils.h"
#import "UGCPriorityBoxArray.h"
    ...
    switch (dimension) {
        ...
        case COMPONENT_GREEN:
            ...
            for (NSInteger i = lower; i <= upper; i++) {
                ...
                NSInteger newColor = [UGCPaletteColorUtils quantizedGreen:color] << (QUANTIZE_WORD_WIDTH + QUANTIZE_WORD_WIDTH)
                | [UGCPaletteColorUtils quantizedRed:color]  << QUANTIZE_WORD_WIDTH | [UGCPaletteColorUtils quantizedBlue:color];
        ...
        case COMPONENT_BLUE:
            ...
            for (NSInteger i = lower; i <= upper; i++) {
                NSInteger color = [_distinctColors[i] intValue];
                NSInteger newColor =  [UGCPaletteColorUtils quantizedBlue:color] << (QUANTIZE_WORD_WIDTH + QUANTIZE_WORD_WIDTH)
                | [UGCPaletteColorUtils quantizedGreen:color]  << QUANTIZE_WORD_WIDTH
                | [UGCPaletteColorUtils quantizedRed:color];
                _distinctColors[i] = [NSNumber numberWithInteger:newColor];
            }
            break;
    }
 ...
 */
- (UGCPaletteSwatch*)getAverageColor{
    NSInteger redSum = 0;
    ...
    for (NSInteger i = _lowerIndex; i <= _upperIndex; i++) {
        ...
        redSum += colorPopulation * [UGCPaletteColorUtils quantizedRed:color];
        greenSum += colorPopulation * [UGCPaletteColorUtils quantizedGreen:color];
        blueSum += colorPopulation * [UGCPaletteColorUtils quantizedBlue:color];
    }
    ...
    redMean = [UGCPaletteColorUtils modifyWordWidthWithValue:redMean currentWidth:QUANTIZE_WORD_WIDTH targetWidth:8];
    greenMean = [UGCPaletteColorUtils modifyWordWidthWithValue:greenMean currentWidth:QUANTIZE_WORD_WIDTH targetWidth:8];
    blueMean = [UGCPaletteColorUtils modifyWordWidthWithValue:blueMean currentWidth:QUANTIZE_WORD_WIDTH targetWidth:8];

    NSInteger rgb888Color = redMean << 2 * 8 | greenMean << 8 | blueMean;
    
    UGCPaletteSwatch *swatch = [[UGCPaletteSwatch alloc]initWithColorInt:rgb888Color population:totalPopulation];
...
- (void)fitBox{
    ...
    for (NSInteger i = _lowerIndex; i <= _upperIndex; i++) {
        ...
        NSInteger r = [UGCPaletteColorUtils quantizedRed:color];
        NSInteger g =  [UGCPaletteColorUtils quantizedGreen:color];
        NSInteger b =  [UGCPaletteColorUtils quantizedBlue:color];
...
@property (nonatomic,strong) UGCPriorityBoxArray *priorityArray;
...
/** specify mode */
@property (nonatomic,assign) UGCPaletteTargetMode mode;
...
-(instancetype)initWithImage:(UIImage *)image{
    ...
    if (self){
        _image = image;
        static dispatch_once_t paletteToken;
        dispatch_once(&paletteToken, ^{
            serialQueue = dispatch_queue_create("com.palette.ugc", DISPATCH_QUEUE_SERIAL);
        });
    }
    return self;
}
...
- (void)startToAnalyzeImage{
    ...
    dispatch_async(serialQueue, ^{
        ...
        for (int pixelIndex = 0 ; pixelIndex < self.pixelCount; pixelIndex++){
            ...
            red = [UGCPaletteColorUtils modifyWordWidthWithValue:red currentWidth:8 targetWidth:QUANTIZE_WORD_WIDTH];
            green = [UGCPaletteColorUtils modifyWordWidthWithValue:green currentWidth:8 targetWidth:QUANTIZE_WORD_WIDTH];
            blue = [UGCPaletteColorUtils modifyWordWidthWithValue:blue currentWidth:8 targetWidth:QUANTIZE_WORD_WIDTH];
        ...
        if (distinctColorCount <= kMaxColorNum){
            ...
            for (NSInteger i = 0;i < distinctColorCount ; i++){
                NSInteger color = 0;
                if (i < self.distinctColors.count) {
                    color = [self.distinctColors[i] integerValue];
                }
                NSInteger population = hist[color];
                
                NSInteger red = [UGCPaletteColorUtils quantizedRed:color];
                NSInteger green = [UGCPaletteColorUtils quantizedGreen:color];
                NSInteger blue = [UGCPaletteColorUtils quantizedBlue:color];
                
                red = [UGCPaletteColorUtils modifyWordWidthWithValue:red currentWidth:QUANTIZE_WORD_WIDTH targetWidth:8];
                green = [UGCPaletteColorUtils modifyWordWidthWithValue:green currentWidth:QUANTIZE_WORD_WIDTH targetWidth:8];
                blue = [UGCPaletteColorUtils modifyWordWidthWithValue:blue currentWidth:QUANTIZE_WORD_WIDTH targetWidth:8];
                
                color = red << 2 * 8 | green << 8 | blue;
                
                UGCPaletteSwatch *swatch = [[UGCPaletteSwatch alloc]initWithColorInt:color population:population];
                [swatchs addObject:swatch];
            }
            
            self.swatchArray = [swatchs copy];
        }else{
            self.priorityArray = [[UGCPriorityBoxArray alloc]init];
            VBox *colorVBox = [[VBox alloc]initWithLowerIndex:0 upperIndex:distinctColorIndex colorArray:self.distinctColors];
            [self.priorityArray addVBox:colorVBox];
            // split the VBox
            [self splitBoxes:self.priorityArray];
            //Switch VBox to Swatch
            self.swatchArray = [self generateAverageColors:self.priorityArray];
        }
...
- (void)splitBoxes:(UGCPriorityBoxArray*)queue{
...
- (NSArray*)generateAverageColors:(UGCPriorityBoxArray*)array{
    NSMutableArray *swatchs = [[NSMutableArray alloc]init];
    NSMutableArray *vboxArray = [array getVBoxArray];
    for (VBox *vbox in vboxArray){
        UGCPaletteSwatch *swatch = [vbox getAverageColor];
        if (swatch){
            [swatchs addObject:swatch];
        }
    }
...
- (void)initTargetsWithMode:(UGCPaletteTargetMode)mode{
    NSMutableArray *targets = [[NSMutableArray alloc]init];
    
    if (mode < VIBRANT_PALETTE || mode > ALL_MODE_PALETTE || mode == ALL_MODE_PALETTE){
        
        UGCPaletteTarget *vibrantTarget = [[UGCPaletteTarget alloc]initWithTargetMode:VIBRANT_PALETTE];
        [targets addObject:vibrantTarget];
        
        UGCPaletteTarget *lightVibrantTarget = [[UGCPaletteTarget alloc]initWithTargetMode:LIGHT_VIBRANT_PALETTE];
        [targets addObject:lightVibrantTarget];

        UGCPaletteTarget *mutedTarget = [[UGCPaletteTarget alloc]initWithTargetMode:MUTED_PALETTE];
        [targets addObject:mutedTarget];
        
        UGCPaletteTarget *lightMutedTarget = [[UGCPaletteTarget alloc]initWithTargetMode:LIGHT_MUTED_PALETTE];
        [targets addObject:lightMutedTarget];

        UGCPaletteTarget *darkVibrantTarget = [[UGCPaletteTarget alloc]initWithTargetMode:DARK_VIBRANT_PALETTE];
        [targets addObject:darkVibrantTarget];

        UGCPaletteTarget *darkMutedTarget = [[UGCPaletteTarget alloc]initWithTargetMode:DARK_MUTED_PALETTE];
...
- (void)findMaxPopulation{
    ...
    for (NSInteger i = 0; i <_swatchArray.count ; i++){
        UGCPaletteSwatch *swatch = [_swatchArray objectAtIndex:i];
...
- (void)getSwatchForTarget{
    ...
    UGCPaletteColorModel *recommendColorModel;
    for (NSInteger i = 0;i<_targetArray.count;i++){
        UGCPaletteTarget *target = [_targetArray objectAtIndex:i];
        [target normalizeWeights];
        UGCPaletteSwatch *swatch = [self getMaxScoredSwatchForTarget:target];
        if (swatch){
            UGCPaletteColorModel *colorModel = [[UGCPaletteColorModel alloc]init];
...
//            colorModel.bodyTextColorString = [swatch getBodyTextColorString];
            ...
            if (!recommendColorModel){
                ...
                if (!_isNeedColorDic){
                    dispatch_async(dispatch_get_main_queue(), ^{
                        self.getColorBlock(recommendColorModel,nil,nil);
                    });
                    return;
                }
            }
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        self.getColorBlock(recommendColorModel,finalColorDic,nil);
    });

}

- (UGCPaletteSwatch*)getMaxScoredSwatchForTarget:(UGCPaletteTarget*)target{
    CGFloat maxScore = 0;
    UGCPaletteSwatch *maxScoreSwatch = nil;
    for (NSInteger i = 0 ; i<_swatchArray.count; i++){
        UGCPaletteSwatch *swatch = [_swatchArray objectAtIndex:i];
...
- (BOOL)shouldBeScoredForTarget:(UGCPaletteSwatch*)swatch target:(UGCPaletteTarget*)target{
...
- (CGFloat)generateScoreForTarget:(UGCPaletteTarget*)target swatch:(UGCPaletteSwatch*)swatch{
    NSArray *hsl = [swatch getHsl];
